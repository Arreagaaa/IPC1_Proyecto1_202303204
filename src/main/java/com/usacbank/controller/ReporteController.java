...
public class ReporteController {
    ...
    public String generarReporteTransacciones(String cui) throws Exception {
        ...
        document.add(new Paragraph(
                "Fecha de generación: " + new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(new Date()), normalFont));
        document.add(new Paragraph("\n"));
        ...
        for (Cuenta cuenta : cuentasCliente) {
            ...
            if (!transacciones.isEmpty()) {
                ...
                for (Transaccion transaccion : transacciones) {
                    ...
                    // Débito
                    cell = new PdfPCell(
                            new Phrase(transaccion.getMontoDebito() > 0 ? df.format(transaccion.getMontoDebito()) : "",
                                    normalFont));
            ...
            if (!transacciones.isEmpty()) {
                ...
                for (Transaccion transaccion : transacciones) {
                    if (transaccion.getMontoCredito() > 0) {
                        ...
                        PdfPCell cell = new PdfPCell(new Phrase(df.format(transaccion.getMontoCredito()), normalFont));
                        cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                        table.addCell(cell);
            ...
            if (!transacciones.isEmpty()) {
                ...
                for (Transaccion transaccion : transacciones) {
                    if (transaccion.getMontoDebito() > 0) {
                        ...
                        PdfPCell cell = new PdfPCell(new Phrase(df.format(transaccion.getMontoDebito()), normalFont));
                        cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                        table.addCell(cell);
